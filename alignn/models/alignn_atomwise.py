...
class ALIGNNAtomWise(nn.Module):
    ...
    def forward(
        ...
        total_atomwises = []
        for g_atomwise in torch.split(atomwise_pred,tuple(g.batch_num_nodes())):
          total_atomwises.append(torch.linalg.vector_norm(torch.sum(g_atomwise,0)))
        result["out"] = torch.stack(total_atomwises)
        result["grad"] = forces
